...
def generar_baseline():
    ...
    print(f"[*] Baseline guardada exitosamente en {BASELINE_PATH}")

def verificar_integridad():
    """Compara los hashes en tiempo real contra la linea base estatica."""
    ...
    for archivo in CRITICAL_FILES:
        ...
        # Caso 2: El hash actual no coincide con la firma registrada (Modificacion)
            ...
            mensaje = f"MODIFICACIÓN DETECTADA en: {archivo} (Hash previo: {baseline[archivo][:10]}... -> Actual: {hash_actual[:10]}...)"
            registrar_evento("ALERTA_INTEGRIDAD", "file_integrity", "CRITICAL", mensaje)
